...
- **SP-FICHA (v8.05):** "➕ dia" saiu das linhas dos tópicos (a barra "Colocar em… → 📅 hoje" faz isso); entrou **✎ ficha** por linha (edita etapa, dados, tarefas) e o chip 📅 do dia junto das listas.
- **SYNC-3VIAS-V1 + SYNC-PUXA-V1 (v8.06):** pergunta dele: *"tá bem integrado iPad e MacBook? vario muito de devices e não posso perder informações"*. Auditoria: o **push** já era automático (1,5–2 s após editar), mas a **puxada** só acontecia no boot, no 🔄 ou ao reconectar — e o merge era por registro inteiro (o Mac desatualizado que mudasse a etapa apagava a lista que o iPad tinha posto no mesmo contato). Correção: (a) **puxada incremental automática** (`syncPuxar`: só linhas com `atualizado` > última puxada) ao voltar pra aba, ao reconectar e a cada 60 s com a aba visível; merge parcial, sem apagar; se ele está digitando/modal aberto, os dados entram e a tela repinta ao soltar. (b) **merge campo a campo** (`fund3`, base em `_lpcBase`/`_bnBase`): o que EU mudei vence no meu campo, o que o OUTRO mudou fica no dele, arrays (listas, interações, hist) recebem os dois lados; vale na puxada (`fundMerge` com base) e **antes do push** (`fund3Servidor` puxa os ids que vão subir e funde). (c) pendentes entram no push mesmo sem diff de snapshot. **Limite:** provado por invariantes puras e portão; teste com 2 aparelhos logados na base real fica com ele (login não existe no portão).

- **SOLIC-TAREFAS-V1 (v8.07):** pedido dele (voz): *"Solicitações são tarefas em andamento — leva esse tópico do Início pra Tarefas. UX tipo Google Tarefas: várias em sequência, editar fácil, conclusão no final — e integrado com ele também."* Seção **📨 Solicitações em acompanhamento** na Agenda & Tarefas (dobrável/organizável como as outras; o card do Início some com a chave ligada). Cada aberta é uma linha: **○ conclui** (com desfazer), **segurado** e **próximo passo** editáveis na própria linha, ✎ abre a ficha. **＋ Adicionar** aceita "segurado · próximo passo", Enter cria e mantém o foco pra próxima. **Google Tarefas:** reaproveita `gtApi`/`GT_LISTA` da Agenda; a marca `#crmlp-so-<id>` nas notas identifica a tarefa (sem coluna nova, vale em qualquer aparelho); aberta → cria/atualiza lá; concluiu no Google → conclui aqui; apagou lá → não recria. Roda silenciosa ao abrir a Agenda (máx. 1×/min) e a cada mudança. **Limite:** a ponte exige Google conectado com escopo de Tarefas (já é o escopo da Agenda); não testável no portão (sem login).
